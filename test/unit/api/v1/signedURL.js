...
test('Post should throw bad implementation if library fails', (t) => {
  // setup stub of library
  const s3urls = td.replace('../../../../lib/s3-urls.js')
  td.when(
    s3urls.puturl()
...
test('Put should throw bad implementation if library fails', (t) => {
  // setup stub of library
  const s3urls = td.replace('../../../../lib/s3-urls.js')
  td.when(
    s3urls.geturl('test123', td.matchers.anything())
  ).thenReject('Couldnt retrieve get URL')
